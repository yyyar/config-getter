...
module.exports = {
    ...
    getConfig: function(defaultPath, opts) {
        ...
        return module.exports.extendConfig(config, opts);
    },

    /**
     * Extends existing config object
     */
    extendConfig: function(config, opts) {
        ...
        for (let cfg of opts.fetchers) {
            ...
            try {
                opts.log('Processing', cfg.type, 'fetcher:', _.values(cfg.opts).join(', '));
                merge(config, fetcher(cfg.opts), opts.replaceArrays, opts.replaceObjects);
            } catch(e) {
                if (opts.ignoreFailedFetchers) {
                    opts.log('Ignoring failed fetcher: ', cfg.type + ' : ' + e.message);
                } else {
